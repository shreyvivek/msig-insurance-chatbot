...
class PolicyScorer:
    ...
    def __init__(self, claims_db=None):
        # Scoring weights (sum should be ~1.0)
        self.weights = {
            "price_value": 0.20,        # 20% - Price competitiveness
            "coverage_adequacy": 0.25,   # 25% - Coverage meets needs
            "risk_match": 0.15,         # 15% - Matches trip risk profile
            "user_preference": 0.10,     # 10% - User preferences/history
            "reputation": 0.10,         # 10% - Provider reputation/trust
            "claims_performance": 0.20  # 20% - Historical claims performance
        }
    ...
    def score_policies(
        ...
        # Get user profile data for claims analysis
        ages = trip_details.get("ages", [])
        if user_profile and not ages:
            # Try to get age from user profile
        ...
        for quote in quotes:
            ...
            # 6. Claims Performance Score (0-100) - NEW
            claims_score, claims_reason, claims_analysis = self._score_claims_performance(
                quote, destination, ages, interests, medical_conditions, activities
            )
            score["scores"]["claims_performance"] = claims_score
            score["claims_analysis"] = claims_analysis
            ...
            total = (
                ...
                claims_score * self.weights["claims_performance"]
            )
    ...
    def _score_claims_performance(
        ...
        """Score based on historical claims performance for this policy"""
        if not self.claims_db:
            return 70, "No claims data available", None
        
        try:
            plan_name = quote.get("plan_name", "")
            if not plan_name:
                return 70, "Policy name not available", None
            
            # Get claims analysis for this policy
            claims_analysis = self.claims_db.analyze_claims_for_policy_scoring(
            ...
            if claims_analysis.get("total_claims", 0) == 0:
                return 70, "No historical claims data for this policy", claims_analysis
            ...
            approval_rate = claims_analysis.get("approval_rate", 0)
            if approval_rate >= 80:
                score += 15
                reasons.append(f"Excellent approval rate: {approval_rate}%")
            elif approval_rate >= 60:
                score += 10
                reasons.append(f"Good approval rate: {approval_rate}%")
            elif approval_rate < 40:
                score -= 15
                reasons.append(f"Low approval rate: {approval_rate}% - many claims denied")
            
            # Average payout (higher suggests better coverage)
            avg_payout = claims_analysis.get("average_payout", 0)
            if avg_payout > 0:
                if avg_payout >= 20000:
                    score += 10
                    reasons.append(f"High average payout: ${avg_payout:,.2f} - good coverage")
                elif avg_payout >= 10000:
                    score += 5
                    reasons.append(f"Reasonable average payout: ${avg_payout:,.2f}")
            
            # Relevance to user profile
            relevance = claims_analysis.get("relevance_score", 0)
            if relevance >= 50:
                score += 10
                reasons.append(f"High relevance to your profile: {relevance:.0f}/100")
            elif relevance >= 30:
                score += 5
                reasons.append(f"Moderate relevance to your profile: {relevance:.0f}/100")
            
            # Claim type match with user interests/conditions
            claim_types = claims_analysis.get("claim_types", {})
            if interests:
                interests_lower = [i.lower() for i in interests]
                if "adventure" in " ".join(interests_lower) and "Medical" in claim_types:
                    score += 5
                    reasons.append("Policy has handled medical claims (relevant for adventure activities)")
            
            if medical_conditions:
                if "Medical" in claim_types:
                    medical_count = claim_types["Medical"].get("count", 0)
                    if medical_count > 10:
                        score += 10
                        reasons.append(f"Policy has extensive experience with medical claims ({medical_count} cases)")
            
            # Penalize if high denial rate
            denied = claims_analysis.get("denied_claims", 0)
            total = claims_analysis.get("total_claims", 1)
            denial_rate = (denied / total * 100) if total > 0 else 0
            if denial_rate > 30:
                score -= 15
                reasons.append(f"High denial rate: {denial_rate:.1f}% - many claims rejected")
            
            reason_text = "; ".join(reasons) if reasons else "Standard claims performance"
            return max(0, min(100, score)), reason_text, claims_analysis
        
        except Exception as e:
            logger.error(f"Error scoring claims performance: {e}", exc_info=True)
            return 70, "Error analyzing claims data", None
    
    def _score_coverage_adequacy(self, quote: Dict, trip_details: Dict, risk_assessment: Dict = None, activities: List[str] = None) -> tuple:
    ...
    def _build_explanation(
        ...
        if claims_reason and "claims_performance" in scores:
            explanation += f"📊 **Claims Performance** ({self.weights['claims_performance']*100:.0f}% weight): {scores['claims_performance']:.1f}/100\n"
            explanation += f"   {claims_reason}\n\n"
